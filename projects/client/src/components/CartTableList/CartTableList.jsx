          ...
          cartDatas.map((value, index) => {
            return (
              <tr key={index} className="text-[#191C1F]  text-[14px]">
                ...
                <td className="PRICE py-[20px]">{`${Number(value.product.product_price).toLocaleString(
                  "id-ID",
                ...
                <td className="TOTAL py-[20px] font-semibold">{`${Number(value.total).toLocaleString(
                  "id-ID",
                  { style: "currency", currency: "IDR" }
                )}`}</td>
              </tr>
